...
if not changeAbundances:
    ...
    metallicities = [0.01, 0.1, 1, 10]
    #metallicities = [100, 1000, 10000]
...
for metallicity in metallicities:
    for c_to_o in c_to_os:
        ...
        filename = f'Asplund2009_CO{c_to_o}_M{metallicity}'
        ...
        if not readMAT:
            matfile = f'../output/{filename}_full.mat'
        ...
        No overwrite, just export smooth
        Write function to export FastChem 3D matrix w/ T-P grid
        Write fuction to read Fastchem 3D matrix w/ T-P grid
        
        56 total models
        Metallicity / CO Ratio / (R to O) exploration
        Metallicity -- (100x) much lower > (10000x) much higher than solar (Coarse = mag)
        CO -- 0.55 Asplund 09, [0, 0.1, 0.3, 0.55, 0.75, 1, 1.25, 1.5]
        
        Change CO ratio, sum of C + O remains solar, metallicity changes
